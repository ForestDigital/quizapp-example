...
--
-- listenQuizScree.lua
-- Same as quizscreen but for audio tests. Plays and audio file
----------------------------------------------------------------------------------
...
function scene:create( event )
	...
	audioFile = audio.loadSound(testData.audio)
	...
	audioImage = display.newImageRect(testData.image, display.actualContentWidth*0.93, display.actualContentHeight*0.65)
	audioImage.anchorX = 0
	audioImage.anchorY = 0
	audioImage:toFront()
	sceneGroup:insert(audioImage)
	contentArea:insert(audioImage)
	audioImage.alpha = 0
...
end local function showImage()
	    	transition.to(audioImage, { alpha=1, time = 2000 })
	    end
...
function scene:show( event )
	...
	transition.to(contentScreen, {y = display.contentHeight/2.2, height = display.actualContentHeight*0.85, time = 1500,
	    ...
	    timer.performWithDelay(5000, audio.play( audioFile, {channel = audioChannel, onComplete=f} ), 1)	
...
function scene:backPressed()
	...
	elseif (question_counter==1) then
		...
		timer.performWithDelay(3000, audio.play( audioFile, {channel = audioChannel} ), 1)	
